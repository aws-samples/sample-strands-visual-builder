...
- CRITICAL: BedrockAgentCoreApp() MUST be empty - NEVER pass agent parameter: `app = BedrockAgentCoreApp()` (NOT `BedrockAgentCoreApp(agent=...)`)
...
```python
@app.entrypoint
def invoke(payload):  # ← MUST be exactly this signature
    """AgentCore entrypoint for Strands agent"""
    user_message = payload.get("prompt", "")  # ← Extract from payload
    result = agent(user_message)  # ← Pass string to Strands agent
    return {"result": str(result)}
```

**PROHIBITED PATTERNS - NEVER USE:**
- ❌ `def handle_request(query: str)` - Wrong parameter type
- ❌ `def handler(user_input: str)` - Wrong function name and parameter
- ❌ `def agent_handler(query: str)` - Wrong function name and parameter
- ❌ `def process_request(payload)` - Wrong function name
- ❌ Any function name other than `invoke`
- ❌ Any parameter other than `payload`
- ❌ Any type annotations on the parameter
- ❌ Passing payload directly to Strands agent

**WHY THIS MATTERS:**
- AgentCore sends JSON payload as dict, not string
- Wrong signature causes "Input prompt must be of type: str" runtime errors
- Deployed agents fail to respond to user requests
- This is the #1 cause of AgentCore deployment failures
