...
export default function MessageRenderer({ content, isStreaming = false, children }) {
  ...
  if (typeof messageContent === 'string') {
      ...
      try {
        ...
        if (parsed.role === 'assistant' && parsed.content && Array.isArray(parsed.content)) {
          if (parsed.content[0]?.text) {
            ...
            console.log('✅ Extracted text from assistant role:', messageContent.substring(0, 100));
          }
        }
        // Handle other common formats
        else if (parsed.content && Array.isArray(parsed.content) && parsed.content[0]?.text) {
          messageContent = parsed.content[0].text;
          console.log('✅ Extracted from content array:', messageContent.substring(0, 100));
        }
        else if (parsed.content && typeof parsed.content === 'string') {
          messageContent = parsed.content;
          console.log('✅ Extracted from content string:', messageContent.substring(0, 100));
        }
        else if (parsed.text) {
          messageContent = parsed.text;
          console.log('✅ Extracted from text field:', messageContent.substring(0, 100));
        }
        else if (parsed.message) {
          messageContent = parsed.message;
          console.log('✅ Extracted from message field:', messageContent.substring(0, 100));
        }
        else if (parsed.result) {
          // Handle nested result structures
          if (typeof parsed.result === 'string') {
            messageContent = parsed.result;
            console.log('✅ Extracted from result string:', messageContent.substring(0, 100));
            ...
            console.log('✅ Extracted from result.content:', messageContent.substring(0, 100));
          } else if (parsed.result.text) {
            messageContent = parsed.result.text;
            console.log('✅ Extracted from result.text:', messageContent.substring(0, 100));
          }
        }
        else {
          console.log('⚠️ JSON parsed but no recognized format found:', Object.keys(parsed));
        }
      } catch (e) {
        ...
        if (messageContent.includes("'role': 'assistant'")) {
          try {
            ...
            console.log('✅ Successfully parsed single-quoted JSON:', parsed);

            if (parsed.role === 'assistant' && parsed.content && Array.isArray(parsed.content)) {
              if (parsed.content[0]?.text) {
                messageContent = parsed.content[0].text;
                console.log('✅ Extracted text from single-quoted JSON:', messageContent.substring(0, 100));
              }
            }
          } catch (e2) {
            console.log('❌ Single-quote JSON parsing also failed:', e2.message);
          }
        }
      }
    }
    ...
    // Handle cases where it might be a stringified object without proper JSON format
      ...
      console.log('🔍 Attempting to extract from malformed JSON string...');
      ...
      for (const pattern of patterns) {
        ...
        if (textMatch && textMatch[1]) {
          messageContent = textMatch[1];
          console.log('✅ Extracted text using regex pattern:', pattern.source, '→', messageContent.substring(0, 100));
          break;
        }
      }
    }
    ...
    if (messageContent.includes("{'role': 'assistant'") || messageContent.includes('{"role": "assistant"')) {
      console.log('🚨 LAST RESORT: JSON parsing completely failed, showing fallback message');
  ...
  const formatContent = (text) => {
    ...
    console.log('🎨 Formatting content with React components:', text.substring(0, 100));
